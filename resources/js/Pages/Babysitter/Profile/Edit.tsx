...
export default function Edit({ user, assignedDevices, flash }: Props) {
  ...
  const submitProfile = (e: React.FormEvent) => {
    e.preventDefault();
    updateProfile(route('babysitter.profile.update'));
  };

  const submitPassword = (e: React.FormEvent) => {
    e.preventDefault();
    updatePassword(route('babysitter.profile.password'), {
      onSuccess: () => resetPassword(),
    });
  };

  const deleteAccount = () => {
    router.delete(route('babysitter.profile.destroy'));
  };
                            ...
                            >
                              <div className="p-6">
                                ...
                                <p className="mt-1 text-sm text-gray-500">
                                  Status:{' '}
                                  <span
                                    className={`font-medium ${device.status === 'active' ? 'text-green-600' : 'text-red-600'}`}
                                  >
                                    {device.status}
                                  </span>
                                </p>
